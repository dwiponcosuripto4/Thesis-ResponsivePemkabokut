$(document).ready(function () {
    ...
    // Menangani penghapusan gambar existing dari server
    $(document).on("click", ".remove-file-btn[data-image]", function (e) {
        e.preventDefault();
        e.stopPropagation();
        ...
        if (
            imageName &&
            confirm("Apakah Anda yakin ingin menghapus gambar ini?")
        ) {
            $.ajax({
                url: "/admin/post/delete-image",
                method: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    image: imageName,
                ...
                success: function (response) {
                    if (response.success) {
                        ...
                        setTimeout(() => imageItem.remove(), 200);
                    } else {
